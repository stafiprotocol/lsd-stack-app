...
import { getMonFactoryAbi, monConfig } from 'config/mon';
import {
  EVM_CREATION_STEPS,
  MON_CREATION_STEPS,
...
const ParameterPage = () => {
  ...
  const btnType = useMemo(() => {
    if (metaMaskAccount && metaMaskChainId !== monConfig.chainId) {
      return 'secondary';
    }
    return 'primary';
  }, [metaMaskAccount, metaMaskChainId, monConfig.chainId]);

  useEffect(() => {
    dispatch(
      setCreationStepInfo({
        steps: MON_CREATION_STEPS,
        activedIndex: 1,
      })
    );
  }, [dispatch]);
  ...
  const { writeAsync: customCustomWriteAsync } = useContractWrite({
    address: monConfig.factoryContract as `0x${string}`,
    abi: getMonFactoryAbi(),
  ...
  const submit = async () => {
    ...
    if (metaMaskChainId !== monConfig.chainId) {
      try {
        switchNetwork && switchNetwork(monConfig.chainId);
      } catch (err: any) {
        console.error(err);
      }
      return;
    ...
    setParamList([
      ...
      {
        name: 'Stable Coins',
        value: monConfig.stableCoins.map((c) => c.name).join(', '),
      },
    ]);
            ...
            <div className="bg-color-bg2 rounded-[.3rem] pb-[.14rem] border-[.01rem] border-color-border1">
              ...
              <div className="w-[5.47rem] mx-auto mt-[.32rem] gap-[.16rem] flex flex-col">
                <InputItem
                  ...
                  placeholder={`Example: StaFi r${monConfig.symbol}`}
                />
                {tokenName.length > 50 && (
                ...
                <InputItem
                  ...
                  placeholder={`Example: r${monConfig.symbol}`}
                />
                {symbol.length > 10 && (
