...
export const UpdateLrtOperatorModal = ({
  open,
  close,
  contractAddress,
  placeholder,
  ...
  const [buttonDisabled, buttonText] = useMemo(() => {
    ...
    if (!value || !validateAddress(value) || !contractAddress || !poolAddr) {
      return [true, 'Submit'];
    }
    return [false, 'Submit'];
  }, [metaMaskAccount, metaMaskChainId, value, contractAddress, poolAddr]);

  const { writeAsync } = useContractWrite({
    address: contractAddress as `0x${string}`,
    abi: getLrtStakeManagerAbi(),
    functionName: placeholder ? 'poolChangeOperator' : 'poolInitOperator',
    args: [],
  });
  ...
  const submit = async () => {
    ...
    try {
      const result = await writeAsync({
        args: [poolAddr, value, ['', BigInt(0)], HashZero],
      });
